...
void DiscoverLayersInPathList(const std::string& path) {
    ForEachFileInPath(path, [&](const std::string& filename) {
        if (filename.find("libVkLayer") == 0) {
...
void DiscoverLayers() {
    auto layerDiscoveryPath =
        pj({android::base::getProgramDirectory(), "lib64"});
    DiscoverLayersInPathList(layerDiscoveryPath);
    std::string vkSdkPath = android::base::getEnvironmentVariable("VULKAN_SDK");
    if (vkSdkPath != "") {
